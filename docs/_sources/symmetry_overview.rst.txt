...
In general we distinguish between four different types of symmetry: reflective, rotational, translational and dihedral.
...
+--------------------------------+---------------------------------------+---------------------+--------+-----------+----------+
| Edge-based symmetry            | Reflective, translational, rotational | :math:`O(m^2)`      | Both   | No        | No       |
+--------------------------------+---------------------------------------+---------------------+--------+-----------+----------+
...
an axis. If a pattern exhibits *rotational* symmetry, it remains
...
it remains unchanged after applying a shift transformation. Additionally, a pattern exhibits
*dihedral* symmetry if it exhibits both reflective and rotational symmetry simultaneously.

**Scope.** The pixel-based symmetry only considers global symmetry axes. As
...
has the advantage that the number of considered axes can be customised. With a lower number
of axes, it is expected that only the strongest, i.e. global, symmetries are considered. With a higher
number of axes, more subtle, local symmetries might be considered as well. The node-based symmetry metric does not offer such
trade-offs. Most other metrics are based on local substructures, with the exception of the pixel-based
symmetry, which considers the whole drawing at once. It is, however, unclear to what extent
...
modifications. Unfortunately, the edge-based symmetry and the pixel-based symmetry are not inherently
...
inherent symmetries of the graph itself :cite:p:`meidiana_automorphism_2022`. The automorphism metric was designed with that exact
goal in mind. The stress- and even-neighborhood-distribution-based approaches consider the structure of the graph as well. The
extent to which it influences the final metric, however, has to be evaluated separately.
...
When focusing only on the faster metrics, we can see that the stress-based approach takes a bit longer compared to the rest. This is due to the additional binary search performed internally to minimize the final metric.
...
Assuming all metrics are capable of measuring some sense of symmetry, we would expect them to exhibit some extent of correlation.

The correlation between all symmetry metrics is depicted below.
...
Some symmetry metrics might highly depend on, e.g., the density or size of a graph.

See the scatter plot below to evaluate whether a specific metric is right for you.
...
.. figure:: images/sym_scatter_2.svg
  ...
  Scatter plot between symmetry metrics and other graph properties
...
.. figure:: images/sym_corr_2.svg
  ...
  Correlation matrix between symmetry metrics and other graph properties
